...
nvm use              # Node.js 24+（见 .nvmrc）
npm install
npx supabase start   # 通过 Docker 启动本地 Postgres、Auth 和 Storage
cp env.example .env.local
npm run dev
```

完整的开发环境搭建指南（包括环境变量、数据库工作流和代码规范），请参见 [CONTRIBUTING.md](CONTRIBUTING.md)（英文）。
...
开发环境搭建、数据库工作流、代码规范、部署指南和 PR 指引请参见 [CONTRIBUTING.md](CONTRIBUTING.md)（英文）。
